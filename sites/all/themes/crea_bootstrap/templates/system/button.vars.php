 ...
 */
function crea_bootstrap_preprocess_button(&$vars) {
  $element = &$vars['element'];

  // Drupal buttons should be of type 'submit'.
  // @see https://www.drupal.org/node/2540452
  //creaDump::debug($element);
  $nosubmit = false;
  foreach($element['#attributes']['class'] as $classes){
    $classesTab = explode(' ', $classes);
    if(in_array('nosubmit', $classesTab)){
      $nosubmit = true;
      break;
    }
  }
