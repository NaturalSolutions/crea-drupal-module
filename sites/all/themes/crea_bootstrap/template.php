...
function crea_bootstrap_preprocess_html(&$variables) {
  global $language;
  ...
  if($language->language == "en"){
    ...
    drupal_add_css(drupal_get_path('theme', 'crea_bootstrap') . '/css/twentytwenty/en.css');
  }
  ...
  && isset($args[1])){
    ...
    if ($node = node_load($nid)){
      if ($field_identite_visuelle = field_get_items('node', $node, 'field_identite_visuelle')){
        ...
        if ($field_type_page = field_get_items('node', $node, 'field_type_page')){
            if (isset($field_type_page[0]['value'])){
                $variables['type_page'] = $field_type_page[0]['value'];
            }
        }
    }
  }
...
function crea_bootstrap_menu_tree__main_menu($variables) {
  return '<ul class="nav navbar-nav">' . $variables['tree'] . '</ul>';
}

function crea_bootstrap_menu_link(array $variables) {
  $element = $variables['element'];
  $sub_menu = '';
  // hack pour ne pas afficher le menu MON ATLAS
  if($element['#theme'] == 'menu_link__main_menu' && $element['#href'] == 'atlas/account'){
...
function crea_bootstrap_breadcrumb($variables) {
  $crumbs = '';
...
function crea_bootstrap_preprocess_field(&$variables) {
...
function crea_bootstrap_status_messages($variables) {
